...
def listen():
    ...
    with sr.Microphone() as s:
        ...
        op=[]
        for i in s:
            if i == 'into' or i =="in to":
                op.append("*")
            elif i == '+' or i == "plus":
                op.append("+")
            elif i == '-' or i == "minus":
                op.append("-")
            elif i == '/' or i == 'by' or i == "bi" or i=='bye':
                op.append("/")
            else:
                op.append(i)
        s=(" ".join(op))
...
def calc(s):
    ...
    var = []
    for i in s:
        if i.isdigit()==True or checf(i)==True:
            var.append(i)
        else:
            if len(op) == 0:
                if i == 'into':
                    ...
                    op.append("-")
                elif i == '/' or i == 'by' or i == "bi":
                    op.append("/")
            else:
                if (i == "into" or i=="") and pri(op[-1], "") == True:
                    a = eval(var.pop())
                    b = eval(var.pop())
                    var.append(str(cal(a, b, op.pop())))
                    op.append("*")
                elif (i == "into" or i=="")and pri(op[-1], "") != True:
                    op.append("*")
                elif (i == "plus" or i=="+" )and pri(op[-1], "+") == True:
                    a = eval((var.pop()))
                    b = eval((var.pop()))
                    var.append(str(cal(a, b, op.pop())))
                    op.append("+")
                    ...
                    op.append("+")
                elif (i == "minus" or i=="-") and pri(op[-1], "-") == True:
                    a = eval((var.pop()))
                    b = eval((var.pop()))
                    var.append(str(cal(a, b, op.pop())))
                    op.append("-")
                    ...
                    op.append('-')
                elif (i == "by" or i=="bi" or i=='/') and pri(op[-1], "/") == True:
                    a = eval((var.pop()))
                    b = eval((var.pop()))
                    var.append(str(cal(a, b, op.pop())))
                    op.append("/")
...
disp2 = Entry(root, font="Verdana 20", fg="black", bg="#abbab1", bd=0, justify=RIGHT, insertbackground="#abbab1", cursor="arrow")
disp2.focus_set()
...
dot_btn = Button(btnrow3, text=" • ", font="Segoe 21", relief=GROOVE, bd=0, command=dot_clicked, fg="white", bg="#333333")
...
conv_btn = Button(btnrow4, text="🎙️", font="Segoe 18", relief=GROOVE, bd=0, command=conv_clicked, fg="white", bg="#333333")
